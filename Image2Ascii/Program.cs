...
namespace Image2Ascii
{
    ...
    {   
        ...
        /**************************************************************************
        * Display help screen if no paramters given
        ***************************************************************************/
        private static void DisplayHelp()
        {
            ...
            Console.WriteLine(" -i<image filename> path and filename to the image. Also accepts web links.");
            Console.WriteLine("   (BMP, GIF, JPG, PNG or TIFF)");
            /*Console.WriteLine(" -Ts Simple conversion (simple char set)");*/ // future option
    ...
    ***************************************************************************/
    public class ASCIIRenderer
    {
        ...
        {
            rtAlgSimple        = 1,         // future option...not used
            rtShowWtdChrs   = 1 << 1,
            rtWtdChrs  = 1 << 2
        }
        ...
        * 
        * PARAMETERS:
        *   filename - This can be either the full pathname to a local file (BMP)
        ***************************************************************************/
        private void ConvertUsingSimpleMethod(string filename)
        {
            ...
            bool isWebsource        = filename.Contains("http");

            try
            {
                if (isWebsource)
                {   // get stream from web source
                    WebClient client = new WebClient();
                    Stream webstream = client.OpenRead(filename_lower);
                    bitmap = new Bitmap(webstream);
                    webstream.Dispose();
                }
                else
                {
                    bitmap = new Bitmap(filename_lower);
                }
